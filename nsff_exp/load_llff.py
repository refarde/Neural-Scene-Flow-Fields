...
def load_nvidia_data(basedir, start_frame, end_frame,
    ...
    sc = 1. if bd_factor is None else 1. / \
        (np.percentile(bds[:, 0], 5) * bd_factor)
...
def create_bt_poses(hwf):
    ...
    # self.targets['K_src'][0, 0, 0]  # Maximum camera translation to satisfy max_disp parameter
    max_trans = max_disp / hwf[2]
    z_shift = -max_trans / 6.  # -12.0

    print(z_shift)

    init_pos = np.arcsin(-z_shift / max_trans) * \
        float(num_frames) / (2.0 * np.pi)
    ...
    for i in range(num_frames):
        x_trans = max_trans * \
            np.sin(2.0 * np.pi * float(i) / float(num_frames))
        y_trans = max_trans * \
            np.cos(2.0 * np.pi * float(i) / float(num_frames)) / \
            2.0  # * 3.0 / 4.0
        # z_shift + max_trans * np.sin(2.0 * np.pi * float(init_pos + i) / float(num_frames))
        z_trans = 0.
...
def render_wander_path(c2w):
    ...
    for i in range(num_frames):
        ...
        y_trans = max_trans * \
            ...
            3.0  # * 3.0 / 4.0
        z_trans = max_trans * \
            np.cos(2.0 * np.pi * float(i) / float(num_frames)) / 3.0
        ...
        ref_pose = np.concatenate([c2w[:3, :4], np.array(
            [0.0, 0.0, 0.0, 1.0])[np.newaxis, :]], axis=0)
