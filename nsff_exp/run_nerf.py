...
def config_parser():
    ...
    parser.add_argument("--final_width", type=int, default=512,
                        help='training image width, default is 512x288')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        if args.no_ndc:
            # np.ndarray.min(bds) #* .9
            near = np.percentile(bds[:, 0], 5) * 0.9
            # np.ndarray.max(bds) #* 1.
    ...
    args.expname = args.expname + \
        '_F%02d-%02d' % (args.start_frame, args.end_frame)
    ...
    for i in range(start, N_iters):
        ...
        if img_i == 0:
            ...
            flow_bwd, bwd_mask = np.zeros_like(
                flow_fwd), np.zeros_like(fwd_mask)
        elif img_i == num_img - 1:
            flow_bwd, bwd_mask = read_optical_flow(args.datadir, img_i,
                                                   args.start_frame, fwd=False)
            flow_fwd, fwd_mask = np.zeros_like(
                flow_bwd), np.zeros_like(bwd_mask)
        else:
        ...
        if TEST:
            ...
            plt.subplot(2, 3, 2)
            plt.imshow(flow_to_image(flow_fwd)/255. *
                       fwd_mask[..., np.newaxis])

            plt.subplot(2, 3, 3)
            plt.imshow(flow_to_image(flow_bwd)/255. *
                       bwd_mask[..., np.newaxis])
        ...
        if chain_5frames:
            ...
            prob_reg_loss += args.w_prob_reg * \
                torch.mean(torch.abs(ret['raw_prob_p2pp']))

            render_loss += compute_mse(ret['rgb_map_pp_dy'],
                                       target_rgb,
                                       weight_map_pp.unsqueeze(-1))

            sf_reg_loss += args.w_sf_reg * \
                torch.mean(torch.abs(ret['raw_sf_p2pp']))
            sf_cycle_loss += args.w_cycle * compute_mae(ret['raw_sf_pp2p'],
                                                        -ret['raw_sf_p2pp'],
                                                        raw_weight_p2pp.unsqueeze(-1), dim=3)

        loss = sf_reg_loss + sf_cycle_loss + render_loss + \
            flow_loss + sf_sm_loss + prob_reg_loss + depth_loss
        ...
        if i % args.i_img == 0 and i > 0:
            ...
            flow_fwd_rgb = torch.Tensor(
                flow_to_image(flow_fwd)/255.)  # .cuda()
            writer.add_image("val/gt_flow_fwd",
                             flow_fwd_rgb, global_step=i, dataformats='HWC')
            flow_bwd_rgb = torch.Tensor(
                flow_to_image(flow_bwd)/255.)  # .cuda()
            ...
            with torch.no_grad():
                ...
                render_flow_fwd_rgb = torch.Tensor(flow_to_image(
                    render_of_fwd.cpu().numpy())/255.)  # .cuda()
                render_flow_bwd_rgb = torch.Tensor(flow_to_image(
                    render_of_bwd.cpu().numpy())/255.)  # .cuda()
                ...
                writer.add_image("val/monocular_disp",
                                 torch.clamp(
                                     target_depth / percentile(target_depth, 97), 0., 1.),
                                 global_step=i, dataformats='HW')
